...
# option_9 = { 'name' : ('-n', '--permissionfilter'), 'help' : 'filter those non-dangerous permissions', 'action' : 'count'}
...
def main(options, arguments):
    if options.shell != None:
        ...
        if options.display != None:
            ...
            fname = options.input[:-4] + "_Permissions.txt"
            f = open(fname, mode='w+')
            
            _a.ianalyze()
            perms_access = _a.get_bc()[0][1].get_analysis().get_permissions( [] )
            # perms_access = _a.get_analysis().get_permissions( [] )
            for perm in perms_access:
                print >> f, perm, ":"
                for path in perms_access[ perm ]:
                    show_Path( _a.get_vm(), path, f )
                    
            n = _a.get_bc()[-1]
            if n == 2:
                _a.ianalyze2()
                perms_access = _a.get_bc()[1][1].get_analysis().get_permissions( [] )
                for perm in perms_access:
                    print >> f, perm, ":"                    
                    for path in perms_access[ perm ]:
                        show_Path( _a.get_vm(), path, f )
        ...
        # elif options.permissionfilter != None:
            # permission_filter(fname)

            # fnameFilter = fname[:-4] + "_Filtered.txt"
            # permission_diff(fnameFilter)
