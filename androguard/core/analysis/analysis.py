...
import re, random, cPickle, collections, difflib
...
def show_Path(vm, path, f):
  ...
  if isinstance(path, PathVar):
    ...
    print >> f, "%s %s (0x%x) ---> %s->%s%s" % (path.get_access_flag(),
                                                info_var,
                                                path.get_idx(),
                                                dst_class_name,
                                                dst_method_name,
    ...
    if path.get_access_flag() == TAINTED_PACKAGE_CALL:
      ...
      print >> f, "%d %s->%s%s (0x%x) ---> %s->%s%s" % (path.get_access_flag(),
                                                        src_class_name,
                                                        src_method_name,
                                                        src_descriptor,
                                                        path.get_idx(),
      ...
      src_class_name, src_method_name, src_descriptor =  path.get_src( cm )
      print >> f, "%d %s->%s%s (0x%x)" % (path.get_access_flag(),
                                          src_class_name,
                                          src_method_name,
                                          src_descriptor,
                                          path.get_idx())
...
def show_Permissions(dx):
    ...
    if n == 1:
        p = dx1.get_permissions( [] )
        
        for i in p:
            print >> f, i, ":"
            for j in p[i]:
                show_Path( dx1.get_vm(), j, f )        
    else:
        p1 = dx1.get_permissions( [] )
        p2 = dx2.get_permissions( [] )
        ...
        for i in p2:
            print >> f, i, ":"
            for j in p2[i]:
                show_Path( dx2.get_vm(), j, f )    
    
    permission_filter(fname)
...
def permission_filter(filename):
    permissionList = ['android.permission.READ_CALENDAR :', \
    ...
    fileReadObj = open(filename, mode='r+')
    filterName = filename[:-4] + "_Filtered.txt"
    fileWriteObj = open(filterName, mode='w+')
    
    fileLineText = fileReadObj.readline()
    while ('' != fileLineText):
        string = fileLineText
        if string[0:-1] in permissionList:
            fileWriteObj.write(string)
            fileLineText = fileReadObj.readline()
            stringNext = fileLineText
            while (cmp(stringNext[0:7], "android") != 0 and cmp(stringNext[0:10], "com.android") != 0):
                fileWriteObj.write(stringNext)
                fileLineText = fileReadObj.readline()
                stringNext = fileLineText
        else:
            fileLineText = fileReadObj.readline()
...
def permission_diff(filename):
    fileReadObj = open(filename, mode='r+').readlines()
    fileReadObj[-1] += "\n"
    compareFileObj = open("SearchBox_7.1_Permissions_Filtered.txt", mode='r+').readlines()
    compareFileObj[-1] += "\n"
    
    diffName = filename[:-4] + "_Diff.txt"
    fileWriteObj = open(diffName, mode='w+')
    
    d = difflib.Differ()
    
    diff = list(d.compare(compareFileObj, fileReadObj))
    
    for line in diff:
        if (line[0] == '-') or (line[0] == '+') or (line[2] == 'a'):
            fileWriteObj.write(line)
